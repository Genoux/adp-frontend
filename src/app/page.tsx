...
function Home() {
  ...
  const createRoomLogic = async (data: {blueTeamName: string, redTeamName: string}) => {
    setLoading(true);
    ...
    const room = await response1.json();
    const mappedBlueTeam = mapToBlueTeamStructure(room.blue);
    setBlueTeam(mappedBlueTeam);

    const mappedRedTeam = mapToRedTeamStructure(room.red);
    setRedTeam(mappedRedTeam);

    const modifiedRoom = {
      ...room.room,
      blue: mapToBlueTeamStructure(room.blue),
      red: mapToRedTeamStructure(room.red),
    };
    ...
    <main className="flex h-full flex-col items-center justify-start">
      {appMode === 'false' ? (
        ...
        <>
          ...
          >
            <div className='mx-auto flex max-w-[980px] flex-col items-center gap-2  md:py-12 md:pb-8 lg:py-18 lg:pb-16'>
          ...
          <RoomCreationForm submit={(data) => createRoomLogic(data as any)}  />
