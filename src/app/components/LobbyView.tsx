...
// Components
const TeamStatus: React.FC<{ isReady: boolean; className?: string }> = ({
  isReady,
  className,
}) => {
  return (
...
const TeamDisplay = ({ team, isCurrentTeam }: TeamDisplayProps) => {
  ...
  return (
    <div
      className={`flex w-full items-center justify-between ${isCurrentTeam ? `bg-${team.color}-600 border border-opacity-50 bg-opacity-5 border-${team.color}-600` : 'border border-zinc-800 bg-zinc-900 bg-opacity-25 '} p-4`}
    >
      <div className="flex h-8 w-full items-center justify-between gap-1">
        <div>
          ...
          {isCurrentTeam && (
            <p
              className={`text-xs text-${team.color} opacity-80`}
            >{`Vous êtes l'équipe ${team.color === 'blue' ? 'bleue' : 'rouge'}`}</p>
          )}
        </div>
  ...
  if (isTeamReady(currentTeam) && !isTeamReady(otherTeam)) {
    return (
      <motion.div
        ...
        animate={{ opacity: 1 }}
        transition={defaultTransition}
        className={clsx('flex gap-1 text-base', className)}
      >
    ...
    <div className="mx-auto flex h-screen flex-col items-center justify-center">
      <div className="w-96 border bg-black bg-opacity-20 p-6">
        ...
        <section className="flex w-full flex-col gap-6">
          ...
          <div className="mx-auto flex h-12 w-60 items-center justify-center">
            <ReadyButton
              otherTeam={otherTeam}
