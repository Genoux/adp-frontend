...
const TeamBans = ({ team }: Team) => {
  return (
    <>
      <div
        className={`flex gap-2 w-full h-[80px]`}>
        {(team.heroes_ban as unknown as Hero[]).map(
          (hero: Hero, index: number) => (
            <div
              key={index}
              className={`h-full w-full overflow-hidden relative rounded-md ${hero && hero.selected ? "bg-draftEmpty" : "bg-draftEmpty opacity-50"
                }`}
            >
              {
                hero && hero.selected ? (
                  hero.name === null ? (
                    // Show SVG when hero.name is null and hero.selected is true
                    ...
                    <div>
                      ...
                      <div
                        ...
                        style={{
                          backgroundImage: `url("/images/champions/splash/${hero.name.toLowerCase()}.jpg")`,
                        }}
                      />
                    </div>
                  )
