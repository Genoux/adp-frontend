...
export async function resetForPlanning(roomid: string) {
  ...
  await supabase.from("rooms").update({ cycle: 0, status: "planning" }).eq("id", roomid);

  // Fetch current heroes_pool from the room
  const { data: room, error: fetchError } = await supabase.from("rooms").select("heroes_pool").eq("id", roomid).single();
  if (fetchError || !room) {
  ...
  const { error: updateError } = await supabase.from("rooms").update({ heroes_pool: updatedHeroesPool }).eq("id", roomid);
  if (updateError) {
  ...
  await supabase.from("teams").update({ heroes_selected: resetSelected, heroes_ban: resetBan }).eq("room", roomid);
}
...
export async function setSelect(roomid: string) {
  await supabase.from("rooms").update({ cycle: 7, status: "select" }).eq("id", roomid);
  await supabase.from("teams").update({ ready: true }).eq("room", roomid);
  await supabase.from("teams").update({ isturn: true }).eq("room", roomid).eq("color", "blue");
  await supabase.from("teams").update({ isturn: false }).eq("room", roomid).eq("color", "red");
}
